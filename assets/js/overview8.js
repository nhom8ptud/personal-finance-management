        ...
        series: [
            {
                ...
                data:[
                    {value:totalIncome, name:'Thu nhập'},
                    {value:totalExpenses, name:'Chi tiêu'},
                    {value:totalSavings, name:'Số dư'}
                ]
            }
        ]
    };
...
function reports(reports) {
    ...
    $(".income-count").text(reports.income.count+" Giao dịch");
    $(".reports-expenses").text(reports.expenses.total);
    $(".expenses-count").text(reports.expenses.count+" Giao dịch");

    
    $(".top-expenses").empty();
    if (Object.keys(reports.expenses.top).length) {
        for (let expense of reports.expenses.top) {
            $(".top-expenses").append('<tr><td>'+expense.title+'</td><td style="float: left;">'+expense.expense_date+'</td><td style="float: right;">'+expense.amount+'</td></tr>');
        }
    }else{
        $(".top-expenses").html('<tr ><td colspan="3" style="text-align:center;">Trống!</td> </tr>');
    }

    $(".top-incomes").empty();
    if (Object.keys(reports.incomes.top).length) {
        for (let income of reports.incomes.top) {
            $(".top-incomes").append('<tr><td>'+income.title+'</td><td style="float: left;">'+income.income_date+'</td><td style="float: right;">'+income.amount+'</td></tr>');
        }
    }else{
        $(".top-incomes").html('<tr ><td colspan="3" style="text-align:center;">Trống!</td> </tr>');
    }
